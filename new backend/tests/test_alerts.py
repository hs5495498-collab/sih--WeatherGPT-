...
class TestCreateAlert:
    ...
    def test_never_claims_to_be_an_official_warning(self):
        # This locally-generated engine has no official IMD/NDMA feed
        # behind it -- every alert it produces must be labelled as an
        # automated advisory, never OFFICIAL_WARNING, so a client can't
        # mistake one for a real government alert.
        alert = alert_service.create_alert(
            risk_type="heat", risk_level="SEVERE", score=95, message="extreme heat"
        )
        assert alert["information_class"] == "AUTOMATED_ADVISORY"
...
class TestAlertsEndpoint:
    ...
    def test_returns_active_alerts_for_severe_weather(
        ...
        # Endpoint-level guarantee: the honesty fields are present and every
        # alert item is explicitly marked as automated, not official.
        assert body["official_provider_active"] is False
        assert all(a["information_class"] == "AUTOMATED_ADVISORY" for a in body["alerts"])
